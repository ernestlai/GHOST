...
var dvh_ghost = (function (dvh) {
    ...
    dvh.ghost.logistic = function (m, x) {
        return m / (1 + Math.pow(Math.E, x));
    };
    ...
    dvh.ghost.TransmissionNetworkModel = function (graph) {
        ...
        TransmissionNode.prototype = {
            reset: function () {
                ...
                this._active = true;
                this._dim = false;
                this.size = dvh.ghost.logistic(40, -6 * (d3.max(d3.values(this.genotypes), function (d) {
                        return d.n_diversity;
                    })));
        ...
        TransmissionLink.prototype = {
            get active() {
                return this._active && this.source.active && this.target.active;
            },
            set active(val) {
                this._active = val;
        ...
        TransmissionNetworkModel.prototype = {
            ...
            _reset: function () {
                this._set_ordinal('group', function (lhs, rhs) {
                    return lhs - rhs;
                }, true);
    ...
    dvh.ghost.TransmissionNetorkPresenter = function (model) {
        var threshold_format = d3.format('.3f');

        model.on('threshold_change.svg_presenter', function (threshold) {
            d3.select('#thresholdslider').property('value', threshold);
            d3.select('#thresholdOutput').html(threshold_format(threshold));
        });
        ...
        model.on('nodes_changed.svg_presenter', function () {
            ...
            svg.selectAll('.node')
                ...
                .attr("d",
                    d3.svg.symbol()
                        .type(function (d) {
                            return d._display_ord.shape;
                        })
                        .size(function (d) {
                            return Math.pow(d.size, 2);
                        }) // svg's size is based on their square area rather than radius.
                )
                .style('visibility', function (d) {
                    return d.active ? 'visible' : 'hidden';
                })
                .attr("fill", function (d) {
                    return d._display_ord.color;
                })
                .style('fill-opacity', function (d) {
                    return d._dim ? 0.4 : 1.0;
                });
